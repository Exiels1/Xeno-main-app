...
# Load environment variables early so any run-time decision can use them
load_dotenv()
...
def save_message(role, content, db_file=DB_FILE):
    conn = sqlite3.connect(db_file)
    cur = conn.cursor()
    cur.execute(
...
def get_conversation_history(limit=20, db_file=DB_FILE):
    conn = sqlite3.connect(db_file)
    cur = conn.cursor()
    cur.execute(
...
def create_app(test_config=None):
    """
    Flask application factory. This lets external servers (or run_public) import create_app().
    """
    ...
    if api_key:
        ...
        client = None  # handlers will react accordingly
    ...
    def chat():
        ...
        bot_reply = None
        if client is None:
            bot_reply = "⚠️ Xeno cannot reach the Groq client because GROQ_API_KEY is not set."
        else:
            try:
...
# === MAIN (smart-run for Render vs local) ===
if __name__ == "__main__":
    # If running on Render (or any environment that sets PORT), use Eventlet WSGI server
